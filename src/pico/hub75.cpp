...
void IRAM_ATTR Hub75::set_color(uint x, uint y, Pixel c) {
    int offset = 0;
...
void IRAM_ATTR Hub75::set_pixel(uint x, uint y, uint8_t r, uint8_t g, uint8_t b) {
    int offset = 0;
    if (x >= width || y >= height) return;
    if (y >= height / 2) {
        y -= height / 2;
    ...
    if (g_width == int32_t(width / 2) && g_height == int32_t(height * 2)) {
        for (int y = start_y; y < g_height; y++) {
            ...
            for (int x = start_x; x < g_width; x++) {
                ...
                back_buffer[offset] = (GAMMA_10BIT[b] << b_shift) | (GAMMA_10BIT[g] << g_shift) | (
                                          GAMMA_10BIT[r] << r_shift);
