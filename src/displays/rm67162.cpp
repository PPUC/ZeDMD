...
static void lcd_send_cmd(uint32_t cmd, uint8_t *dat, uint32_t len) {
  ...
  WriteComm(cmd);
  if (len != 0) {
    for (int i = 0; i < len; i++) WriteData(dat[i]);
  }
#endif
}
...
#endif
    {
      lcd_send_cmd(lcd_init[i].cmd, (uint8_t *)lcd_init[i].data,
                   lcd_init[i].len & 0x7f);

      if (lcd_init[i].len & 0x80) delay(120);
    }
  }
}
...
void lcd_fill(uint16_t xsta, uint16_t ysta, uint16_t xend, uint16_t yend,
  ...
  uint16_t *color_p =
      (uint16_t *)heap_caps_malloc(w * h * 2, MALLOC_CAP_INTERNAL);
...
void IRAM_ATTR lcd_PushColors(uint16_t x, uint16_t y, uint16_t width,
